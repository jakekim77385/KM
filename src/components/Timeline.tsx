...
const initialTimeline: Phase[] = [
  {
    ...
    tasks: [
      ...
      { text: '드림 스쿨 Top 10 리스트 확정', done: false },
      { text: 'SAT 재응시 여부 결정 (1570 수학 800 만점 — 재응시 불필요, 에세이 집중 권장)', done: false },
    ]
  },
  {
    id: 'may26', month: '5월', year: '2026', title: 'AP 시험의 달 🔥', status: 'upcoming',
    tasks: [
      { text: 'AP Calculus BC 시험', done: false, urgent: true },
      { text: 'AP Physics 1 시험', done: false, urgent: true },
      { text: 'AP Chemistry 시험', done: false, urgent: true },
      { text: 'AP English Language 시험', done: false, urgent: true },
...
export default function Timeline() {
  const [timeline, setTimeline] = useState<Phase[]>(() => {
    if (typeof window !== 'undefined') {
      const stored = localStorage.getItem('kkumi-timeline');
  ...
  const saveTimeline = (updated: Phase[]) => {
    setTimeline(updated);
    if (typeof window !== 'undefined') localStorage.setItem('kkumi-timeline', JSON.stringify(updated));
  };
